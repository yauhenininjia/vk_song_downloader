...
class SongsController < ApplicationController
  ...
  def download_song_locally(url, filename = 'noname')
    logger.info "Start  downloading #{filename} from #{url.truncate(60)}"
    start = Time.now
    tmp_file = open(url)
    logger.info "Finish downloading #{filename} from #{url.truncate(60)}. Total #{number_to_human_size(tmp_file.length)} in #{(Time.now - start).round(2)}s"
    tmp_file
  end
end
